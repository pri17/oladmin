...
angular.module('ciApp').controller('setupOlfsCtrl', ['$scope', '$mdToast', 'ServiceSetup', '$filter', '$stateParams', '$interval', function($scope, $mdToast, ServiceSetup, $filter, $stateParams, $interval){
	...
	$scope.initAction = function() {
		$scope.setCommand('olfsinit', $scope.getInitResponse);
		$scope.action.done = 0;
	};
	...
	$scope.getInitResponse = function() {
		var promesa = ServiceSetup.getCmd('olfsinit');
		promesa.then(function(data) {
			var now = new Date();
			if(data.status == 'OK')
			{
		...
		function(error) {
			console.log("getInitResponse error: " + error);	
		});
	};

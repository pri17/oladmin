...
class ModelBase
{
    ...
    public function connect() {
	    ...
	    $db_name = $di->getConfig()->leveldb->filePath;
	    if($this->db == null)
	    {
		    $this->db = new LevelDB($db_name);
...
//*/
    ...
    public function scan_ll($key_start, $key_end, $limit = 0)
    {
	    ...
	    while($it->valid())
	    {
		    $key = $it->key();
		    $value = $it->current();
		    if($key_end != '' && strcmp($key, $key_end) >= 0)
			    break;
		    $out[$key] = $value;
		    $count++;
